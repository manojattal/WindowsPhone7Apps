...
//var postCNDServer = 'http://localhost:18080/share';
...
function isOldIE()
{
	if (navigator.appVersion.indexOf('MSIE 7.')>0) return true;
...
{
//	serverRequest('/metadata?publisherKey='+publisherKey+'', function(data)
//	{
	var data = metadata;
...
function serverRequest(requestString, onComplete) {
	...
	var request = pwjQ.ajax({
		...
		dataType: "jsonp",
		url: poStCoreServer+requestString+'&random='+getRandom(),
...
function createCommonLogUrlPart(eventType)
{
	...
	if (!isOptout)
	{
		result = result+'&pt='+encodeURIComponent(shareTitle)
...
function updateCounterValue(noIncrement)
{
	...
	noIncrement = (noIncrement)? 'true':'false';
	serverRequest('/counterValue?publisherKey='+publisherKey+'&sharer=all&url='+shareUrl+'&noincrement='+noIncrement+"&random="+getRandom(), function(data){
...
/*
function openInNewWindow(windowURL,sharer) 
{
	var leftVal = (screen.width- 600) / 2;
	var topVal = (screen.height - 300 ) / 2;
	var url = poStClassicServer+"/shareRedirect.jsp?url="+encodeURIComponent(windowURL);
	var result = window.open(url, '_newTab','location=yes,status=no,scrollbars=yes,channelmode=yes');
	if (result == null)
	{
		alert ("Your sharing window is blocked. Please disable pop-up blocker for this site.");
		return;
	}
	emitLogPixel(createShareLogUrl(sharer));		
}
...
function showEmailWindow()
{
	...
	pwjQ('#ra1-pw-share-container').fadeIn(300, function()
	{
...
function showMorePopup(e,buttonDiv)
{
	...
	center(addMorePopup);
	addMorePopup.fadeIn(300, function()
	{
...
{	
	  ...
	  var addMorePopupIn = pwjQ('<div class="ra1-pw-addMorePopup-i" ></div>');
...
{	
	  ...
	  var addMoreMorePopupIn = pwjQ('<div class="ra1-pw-addMorePopup-i" ></div>');
...
function setupGooglePlus(buttonDiv)
{
	...
	if (sizeAspect == 32)
	{
	   googlePlusDiv = pwjQ('<span class="ra1-pw-googlePlus-32"><g:plusone count="false"></g:plusone></span>');
	}
	else
	if (sizeAspect == 24)
	{
	       googlePlusDiv = pwjQ('<span class="ra1-pw-googlePlus-24"><g:plusone count="false"></g:plusone></span>');
	}
	else
	if (sizeAspect == 16)
	{
		googlePlusDiv = pwjQ('<span class="ra1-pw-googlePlus-16"><g:plusone size="small" count="false"></g:plusone></span>');
	}	
